...
class capacity_iterator():
    ...
    # * for bounded degree search, set mindeg and maxdeg to desired values (additionally,
    # set m_degrees to False to switch off thresholding)
    # * for bounded variables search, set minvars and maxvars  to
    # desired values (additionally, set m_variables to False to switch off thresholding)
    # * for bounded window search,set minwindow and maxwindow (additionally, set m_window and m_windowpos to False
    # to switch off thresholding)
    ...
    def collect(self, inputs, states):
        # inputs=inputs[:,0]
        self.reset()
        donext = True
    ...
    #    def score_PCA(self,score,threshold):
    #        if not(self.use_scores):
    #            print('WARNING: scoring has been switched off, any further scores will be ignored')
    #        if self.scored:
    #            print('WARNING: duplicate scoring of individual, any further scores will be ignored')
    ...
    #            self.cumscore+=score
    #            if self.profile:
    #                self.tags.append({'degree': self.degree,
    ...
    #                                  'delay': self.delay,
    #                                  'score': score,
    #                                  'PCA_scores':self.PCAscore,
    #                                  'PCA_total_score':self.PCAtotalscore,
